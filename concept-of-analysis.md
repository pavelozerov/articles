...
- When analysis is ready developers can start to work on **feature implementation** using tickets from **development backlog.**
- When development finished, there is a QA phase when engineers prepare **automated tests** that are implementation of the behavioral scenarios created during the Feature Mapping process.
- When functional testing is done, feature moves to **acceptance testing**, where analyst and business have to make sure that current implementation does exactly the same what is in the business requirements (Figure 9).
...
First, scenario should be written in a simple way to be easily readable and understandable. To achieve this BDD uses neat Gherkin syntax. But it's not enough. Within your company all the people, who work with BDD should agree on consistent structure and naming convention to be used.
